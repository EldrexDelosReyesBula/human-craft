...
param (
    [string]$Target = "all" # Options: all, human-craft, security-privacy-auditor, agent-engineering
)
...
function Install-Skill {
    ...
    $SkillDir = "$GlobalSkillsDir\$SkillName"
    $TargetFile = "$SkillDir\SKILL.md"
    ...
    if (Test-Path $SourcePath) {
        Copy-Item $SourcePath -Destination $TargetFile -Force
...
if ($Target -eq "all" -or $Target -eq "human-craft") {
    $hcSource = "$CurrentScriptDir\SKILL.md"
...
if ($Target -eq "all" -or $Target -eq "security-privacy-auditor") {
    $secSource = "$CurrentScriptDir\skills\security-privacy-auditor\SKILL.md"
...
if ($Target -eq "all" -or $Target -eq "agent-engineering") {
    foreach ($skill in $agentSkills) {
        $skillSource = "$CurrentScriptDir\skills\$skill\SKILL.md"
